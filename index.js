...
const getSocialMediaData = async () => {
  ...
  const socialMediaData = await page.evaluate(() => {
    const socialMediaLinks = document.querySelectorAll('.mt-4 a[aria-label^="profile followers"]');
    const data = Array.from(socialMediaLinks, link => {
      ...
      let count = parseFloat(countText.replace(/k/i, 'e3').replace(/m/i, 'e6').replace(/,/g, ''));
      if (isNaN(count)) {
